...
const STRIPE_PRICE_MONTHLY = 'price_1TeeAtV051r3HhJO9wNlaMEq';
const STRIPE_PRICE_ANNUAL  = 'price_1TeeAvV051r3HhJOSkNz6g2D';
...
function updateTopbar() {
  ...
  if (S.role === 'business') {
    ...
    const manageBtn = document.getElementById('biz-manage-sub-btn');
    const proBadge = document.getElementById('biz-pro-badge');
    const isPro = APP.bizSubscription?.status === 'active' || APP.bizSubscription?.status === 'canceling';
    if (getProBtn) getProBtn.style.display = isPro ? 'none' : 'flex';
    if (manageBtn) manageBtn.style.display = isPro ? 'flex' : 'none';
