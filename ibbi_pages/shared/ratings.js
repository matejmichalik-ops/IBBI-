...
function selectStar(n) {
  _selectedStar = n;
  const labelKeys = ['','rating-1','rating-2','rating-3','rating-4','rating-5'];
  document.getElementById('star-val').textContent = t(labelKeys[n]) + ` (${n}/5)`;
  [1,2,3,4,5].forEach(i => {
...
async function submitRating(targetUserId, campId, targetRole) {
  if (!_selectedStar) { toast(t('rating-select-stars'), 'warning'); return; }
  ...
  toast(t('rating-submitted'), 'success');
}
